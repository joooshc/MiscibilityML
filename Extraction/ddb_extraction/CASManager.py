...
class CASManager:
    def __init__(self):
        self.cas_smiles_file = 'cas_smiles_dict.json'
        self.cas_pubchem_props_file = 'cas_pubchem_props_dict.json'
        self.cas_rdkit_descriptors_file = 'cas_rdkit_descriptors_dict.json'
    
    def smiles_gen(self):
        # Generating a dataframe from the JSON file
        jcsv = JSON_to_CSV()
        jcsv.load_data("Datasets/DDBscrapeddata/IUPAC_vap_output.json")
    ...
    def merge_dicts_to_df(self, df, cas_pubchem_props_dict, cas_rdkit_descriptors_dict):
        ...
        df1.to_csv('compound1_dataset_raw.csv', index=False)
        df2.to_csv('compound2_dataset_raw.csv', index=False)
        df_diff.to_csv('diff_dataset_raw.csv', index=False)
