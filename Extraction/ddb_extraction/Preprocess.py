...
class Preprocessing:
    ...
    def log_scaling(dataset):
        ...
        removed_cols = dataset.iloc[:, 0:4]
        dataset_scaled = pd.concat([removed_cols, dataset_scaled], axis=1)
        return dataset_scaled, dataset_scaled.iloc[:, 4]
    ...
    def load_json(json_filepath):
        with open("Datasets/IUPAC/FeatureEditing.json") as json_file:
            data = json.load(json_file)
        return data['to remove']

    @staticmethod
    def process_dataset(dataset, num_features, plot_hist, plot_scaled_hist, json_filepath):
        warnings.filterwarnings("ignore", category=RuntimeWarning) 

        # Load the JSON file and get the list of columns to remove
